...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='plan_limits' AND column_name='target_loss_min') THEN
    ALTER TABLE plan_limits ADD COLUMN target_loss_min FLOAT DEFAULT 10;
  END IF;
...
ON CONFLICT (plan) DO UPDATE SET
  max_positions = 99,
  max_global_exposure_usd = 9999999,
