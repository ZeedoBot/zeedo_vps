...
export function LinksClient() {
  ...
  const internalLinks = useMemo(() => {
    ...
    const fromTiktokBio = utm_source === "tiktok" && utm_medium === "bio";

    return LINKS.map((l) => {
      if (l.external) return l;
      if (l.href !== "/acesso-antecipado") return l;

      if (!fromTiktokBio) return l;
      const qs = new URLSearchParams({
        utm_source: "tiktok",
        utm_medium: "bio",
        utm_campaign: utm_campaign || "azevedocrypto",
      }).toString();
      return { ...l, href: `/?${qs}` };
    });
  }, [searchParams]);
